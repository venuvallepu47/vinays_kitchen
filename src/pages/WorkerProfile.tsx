...
export function WorkerProfile() {
    ...
    const handleDeletePayment = async (pid: number) => {
        ...
        try { await api.delete(`/salary-payments/${pid}`); toast('Deleted'); fetchData(); }
        catch { toast('Failed', 'error'); }
    };

    const handleDelete = async () => {
        if (!confirm('Remove this worker from active list?')) return;
        ...
        <div className="h-dvh bg-slate-50 flex flex-col max-w-md mx-auto shadow-xl overflow-hidden">
            <TopBar title={worker.name} showBack rightAction={
                <div className="flex items-center gap-1">
                    ...
                    <button onClick={handleDelete} className="p-2 rounded-full hover:bg-danger-50 text-danger-500"><Trash2 size={18} /></button>
                </div>
            } />
                        ...
                        <div className="space-y-2">
                            {payments.length === 0 ? (
                                ...
                                <div key={p.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm px-4 py-3.5 flex items-center gap-3">
                                    ...
                                    <button onClick={() => handleDeletePayment(p.id)} className="p-1.5 rounded-lg hover:bg-danger-50 text-slate-300 hover:text-danger-500"><Trash2 size={14} /></button>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            {/* Edit Modal */}
            <Modal isOpen={showEdit} onClose={() => setShowEdit(false)} title="Edit Worker">
                <form onSubmit={handleEdit} className="space-y-4">
                    <div><label className={labelCls}>Name</label><input required value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} className={inputCls} /></div>
                    <div><label className={labelCls}>Phone</label><input value={editForm.phone} onChange={e => setEditForm(f => ({ ...f, phone: e.target.value }))} className={inputCls} /></div>
                    <div><label className={labelCls}>Daily Salary (₹)</label><input required type="number" min="0" step="0.01" value={editForm.salary_per_day} onChange={e => setEditForm(f => ({ ...f, salary_per_day: e.target.value }))} className={inputCls} /></div>
                    <button type="submit" className="w-full py-3.5 bg-primary-600 text-white font-bold rounded-2xl">Update Worker</button>
                </form>
            </Modal>

            {/* Pay Modal */}
            <Modal isOpen={showPay} onClose={() => setShowPay(false)} title="Record Salary Payment">
                <div className="mb-4 bg-slate-50 rounded-xl p-3 flex justify-between">
                    <p className="text-xs text-slate-500 font-bold">Balance Due</p>
                    <p className={cn('text-sm font-black', balance > 0 ? 'text-danger-600' : 'text-success-600')}>{formatCurrency(Math.abs(balance))}</p>
                </div>
                <form onSubmit={handlePay} className="space-y-4">
                    <div><label className={labelCls}>Amount (₹) *</label><input required type="number" min="0.01" step="0.01" placeholder="0" value={payForm.amount} onChange={e => setPayForm(f => ({ ...f, amount: e.target.value }))} className={inputCls} /></div>
                    <div><label className={labelCls}>Payment Date</label><input type="date" value={payForm.payment_date} onChange={e => setPayForm(f => ({ ...f, payment_date: e.target.value }))} className={inputCls} /></div>
                    <div><label className={labelCls}>Notes</label><input placeholder="e.g. Weekly salary" value={payForm.notes} onChange={e => setPayForm(f => ({ ...f, notes: e.target.value }))} className={inputCls} /></div>
                    <button type="submit" className="w-full py-3.5 bg-success-600 text-white font-bold rounded-2xl">Record Payment</button>
                </form>
            </Modal>
        </div>
    );
